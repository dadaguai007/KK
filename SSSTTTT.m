...
for index=1:length(Eb_N0_dB)
    ...
    alpha=0.02;
    for j=1:10
        recoverI=iteraElimate_single(signal+Dc,pd_input,fs,alpha,Dc,Vdither(1));
        % 对信号进行切分，并提出全部信号
...
%         [ReceivedSignal,dc]=Receiver.Preprocessed_signal(totalPortion1);
        ...
        % 信号存储
        mat_E_signal(j,:)=ReceivedSignal;
        mat_I_signal(j,:)=recoverI;
    end
    [ber_total1(index),dex]=min(berSIC);
    
    Signal=mat_E_signal(dex,:);
    Signal_I=mat_I_signal(dex,:);
    ...
    alpha=0.02;
    for j=1:10
        recoverI=iteraElimate_beat(signal+Dc,pd_input,fs,alpha,Dc,Vdither(1));
        % 对信号进行切分，并提出全部信号
        [DataGroup1,totalPortion1]=Receiver.Synchronization(recoverI);
        ...
        Receiver.Nr.nTrainSym=100;
        Receiver.Button.Display='on';
        [berSIC2(j),num_total(j)]=Receiver.Cal_BER(ReceivedSignal);
        pd_input=recoverI;
        re_signal=ReceivedSignal;
        % 信号存储
%         mat_E_signal(j,:)=ReceivedSignal;
    ...
    [ber_total2(index),dex]=min(berSIC2);
