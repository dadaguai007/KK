...
if 1
    ...
    % 初始化设置
    Eb_N0_dB=10:30;
    ber_total2=zeros(length(Eb_N0_dB),1);
    num_total2=zeros(length(Eb_N0_dB),1);
    WB = OCG_WaitBar(length(Eb_N0_dB));
    for index=1:length(Eb_N0_dB)
        % 噪声
        noise=EbN0_dB(sigRxo,Eb_N0_dB(index));
        ...
        [ReceivedSignal,~]=Receiver.Total_Preprocessed_signal(pd_receiver);
        % BER 计算
        [ber_total2(index),num_total2(index)]=Receiver.Cal_BER(ReceivedSignal);
...
BER=[ber_total.';ber_total1;ber_total2.'];
LengendArrary=["80km w/o ","80km w SIC",'80km SSB'];
