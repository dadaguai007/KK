...
if 1
    ...
    % 重新计算E2和E1
    Rece_remove_dc=ReceivedSignal-Dc;
    E1=2*dc*(VbI+VbQ);
    E2=2*real(Rece_remove_dc).*(VbI+VbQ);
    
    alpha=1;
    ipd_error=alpha*(E1+E2);

    ipd_pd=ipd_btb-ipd_error;
end


[E,fre]=mon_ESA(ipd_error,fs,0);

[E_pd,fre]=mon_ESA(ipd_btb,fs,0);
[If_pd,fre]=mon_ESA_flag(ipd_btb,fs,0);
% [If_dbm_iter,fre]=mon_ESA_flag(ipd_pd,fs,0);
Efout=E-E_pd;
