...
Alfa=[0,0.02,0.05,0.08,0.1];
PHI=[0.878,0.86,0.838,0.793,0.76];
for mm=1:length(Alfa)
    alfa1=Alfa(mm);
    V1=alfa1*As;% I路dither幅度 双边带下两路的dither幅度
    V2=alfa1*As;% Q路dither幅度
    V=[V1,V2];
    ...
    if 1
        ...
        for index=1:length(Eb_N0_dB)
            ...
            % BER 计算
            [ber_total(mm,index),num_total(mm,index)]=Receiver.Cal_BER(ReceivedSignal);
            if mm == 1
                continue;
            else
                disp('进入循环');
                ...
                ber_total_iter(mm,index)=min(ber_total1);
                ber_total_iter_Group(mm,index)=min(ber_group);
...
BER=[ber_total(1,:);...
    ber_total(4,:);ber_total_iter(4,:);ber_total(5,:);ber_total_iter(5,:);...
    ber_total(3,:);ber_total_iter(3,:);...
    ber_total(2,:);ber_total_iter(2,:);];
LengendArrary=["80km w/o 0%Vpi",...
    "80km w/o 8%Vpi","80km w SIC 8%Vpi",...
    "80km w/o 10%Vpi","80km w SIC 10%Vpi","80km w/o 5%Vpi","80km w SIC 5%Vpi",...
    "80km w/o 2%Vpi","80km w SIC 2%Vpi",];
