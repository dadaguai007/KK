...
% 文件存储路径
datapath='Output\BTB_SIC_Remod_best_alpha_40k';
...
for index=1:length(Eb_N0_dB)
    ...
    for j=1:1
        ...
        for Idx=1:k
            ...
            Receiver.Nr.nTrainSym=50;
            % 解码运算
            [~,~,~,data_qam_Idx,~]=Receiver.Demodulation(selectSignal);
            % 转换为矩阵形式
            data_mat_Idx=reshape(data_qam_Idx,nn.nModCarriers,[]);
            mat_signal_Idx{Idx}=data_mat_Idx;
            % 存储kk
